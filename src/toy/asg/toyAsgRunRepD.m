...
% per bin (pair)
[Acc, Recall, Obj] = zeross(nAlg, nBin);
prCIn('bin', nBin, 1);
for iBin = 1 : nBin
    prC(iBin);
    ...
    for iAlg = 1 : nAlg
        ...
        elseif strcmpi(algs{iAlg}, 'AGM')
            asg = agm_syn(K,Ct,asgT, pars{:});
        elseif strcmpi(algs{iAlg}, 'OURS')
            asg = agmnc_syn(K,Ct, gphs,asgT, pars{:});
        else
